...
public class FeedPublishRequestParam extends RequestParam implements Parcelable {
	...
	public FeedPublishRequestParam(String name, String description, String url,
			String imageUrl, String caption, String actionName,
			String actionLink, String message) {
		this.name = name;
	...
	public static final Parcelable.Creator<FeedPublishRequestParam> CREATOR = new Parcelable.Creator<FeedPublishRequestParam>() {
	 ...
	 * 
	 * @return Feed长度的状态，详见{@link FeedPublishRequestParam}中的定义
	 */
	private int checkFeed() {
		int flag = FeedPublishRequestParam.NORMAL_LENGTH;
		if (name != null
				&& name.length() > FeedPublishRequestParam.NAME_MAX_LENGTH) {
			flag |= FeedPublishRequestParam.NAME_TOO_LONG;
			//System.out.println("checkFeed name:" + name.length());
		}

		if (description != null
				&& description.length() > FeedPublishRequestParam.DESCRIPTION_MAX_LENGTH) {
			flag |= FeedPublishRequestParam.DESCRIPTION_TOO_LONG;
			//System.out.println("checkFeed description:" + description.length());
		}

		if (caption != null
				&& caption.length() > FeedPublishRequestParam.CAPTION_MAX_LENGTH) {
			flag |= FeedPublishRequestParam.CAPTION_TOO_LONG;
			//System.out.println("checkFeed caption:" + caption.length());
		}

		if (actionName != null
				&& actionName.length() > FeedPublishRequestParam.ACTION_NAME_MAX_LENGTH) {
			flag |= FeedPublishRequestParam.ACTION_NAME_TOO_LONG;
			//System.out.println("checkFeed actionName:" + actionName.length());
		}

		if (message != null
				&& message.length() > FeedPublishRequestParam.MESSAGE_MAX_LENGTH) {
			flag |= FeedPublishRequestParam.MESSAGE_TOO_LONG;
			//System.out.println("checkFeed message:" + message.length());
		}
	...
	private int checkWidgetFeed() {
		...
		if (caption != null
				&& caption.length() > FeedPublishRequestParam.CAPTION_MAX_LENGTH) {
			flag |= FeedPublishRequestParam.CAPTION_TOO_LONG;
			//System.out.println("caption:" + caption.length());
		}

		if (actionName != null
				&& actionName.length() > FeedPublishRequestParam.ACTION_NAME_MAX_LENGTH) {
			flag |= FeedPublishRequestParam.ACTION_NAME_TOO_LONG;
			//System.out.println("actionName:" + actionName.length());
		}

		if (message != null
				&& message.length() > FeedPublishRequestParam.WIDGET_MESSAGE_MAX_LENGTH) {
			flag |= FeedPublishRequestParam.MESSAGE_TOO_LONG;
