...
public class Connect extends Thread {
	...
	*/
	 ...
	 * 
	 */
	private void setToastError(String msg) {
//		Debug.d("setError:" + msg);
		if (this.progressListener != null) {
			// Debug.d("setProgress:" + progress);
			this.progressListener.onToastError(msg);
		}
	}
	 ...
	 * 
	 */
	private void setDialogError(String msg) {
//		Debug.d("setError:" + msg);
		if (this.progressListener != null) {
			// Debug.d("setProgress:" + progress);
			this.progressListener.onDialogError(msg);
		}
	}
...
//		Debug.d("cancelHTTP:" + bCancel);
		synchronized (this) {
			...
			if (p_Connect != null) {
				try {
					...
					// #if DEBUG
					// System.out.println("Connect Cancel failed:" +
					// e.toString());
					// #endif
				}
			}
	...
	private static final int RETRYCOUNT = 0;// 不进行重试
	...
	public void run() {
		...
		while (bRun) {
			// System.out.println("Connect run....1111");
			synchronized (this) {
				bConnected = true;
				nTrying = 0;
			}
			// System.out.println("Connect run....2222");
			if (inputTable.isEmpty()) {// Nothing to process
				...
				// System.out.println("Connect run....33333");
				while (e != null && e.hasMoreElements()) {
					// System.out.println("Connect inputTable.size():"
					// + inputTable.size());
					String strUrl;
					...
					synchronized (this) {
						...
						// System.out.println("Connect  strUrl:" + strUrl);
						...
						if (o instanceof byte[]) {
							...
							if (uploadData.nType == 2)
								strType = uploadData.strType + "; boundary="
					...
					do {
						...
						result = readWebData(strUrl302, proxyMode, uploadData,
								postData, strType, length);// ,
						// toFile);
						// Debug.d("bCancel=" + bCancel);
						// System.out.println("Connect run....5555");
						System.out.println("httpResultCode:" + httpResultCode);
						if (httpResultCode == java.net.HttpURLConnection.HTTP_MOVED_TEMP) {
							...
							// Debug.d("\n");
							synchronized (this) {
								System.out.println("remove:" + strUrl);
...
//						if (nCount > 0)
//							setToastError("重试网络连接:" + nCount);
						if (nCount == 0)
						{
							setDialogError("网络连接失败，请检查网络是否正常！");
							// Max retry count achieved, re detect network.
							nCurrentStatus = 0;
...
//		Debug.d("url:" + url);
		// System.out.println("Connect readWebData url:" + url);
		byte[] result = null;
		bCancel = false;
		synchronized (this) {
			p_Connect = null;
		...
		try {
			...
			// //////////////////////////////
			// 文件名有空格等的处理
			int ll = newUrl.indexOf('?');
			if (ll > 0) {
				int bb = newUrl.lastIndexOf('/', ll);
				if (bb > 0) {
					String tmpUrl = newUrl.substring(0, bb + 1);
					tmpUrl += urlEncode(newUrl.substring(bb + 1, ll));
					tmpUrl += newUrl.substring(ll);
			...
			// System.out.println("Connect p_host:" + p_host);
			if (modeCmwap) {
				p_Proxy = new URL("http://10.0.0.172:80" + p_Url.getFile());
			}
			synchronized (this) {
				// System.out.println("Connect readWebData:111111111");
				if (p_Proxy != null)
				...
				p_Connect.setConnectTimeout(30 * 1000);// min*second*mills
				p_Connect.setReadTimeout(30 * 1000);
			}
			// HttpURLConnection的getResponseCode在第二次请求的时候总是返回-1的问题
			// 遇到过在某些版本的手机上，使用HttpURLConnection第二次请求接口或者网络的时候非常非常的慢，最后还会返回个-1的现象。
			...
			// p_Connect.setRequestProperty("keepAlive", "false");
			p_Connect.setUseCaches(false);
			p_Connect.setRequestProperty("User-Agent", STRINGUSERAGENT);
			// p_Connect.setRequestProperty("Accept-Encoding", "gzip, deflate");
...
//			}
			...
			// System.out.println("Connect readWebData:333333333");
			p_Connect.setDoInput(true);
			p_Connect.setDoOutput(true);
			// System.out.println("Connect readWebData:44444444");
			if (postData != null && postData.length > 0) {
				p_Connect.setDoOutput(true);
				...
				if (postData.length > 2000) {// Progress only for > 2000 and
					...
					while (pos < postData.length) {
						if (pos + len > postData.length) {
							len = postData.length - pos;
						}
						os.write(postData, pos, len);
						if (bCancel) {
							setProgress(0);
							break;
						} else
							setProgress(pos * 100 / postData.length);
						pos += len;
				...
				os = p_Connect.getOutputStream();
				// 写头数据
				os.write(uploadData.headerData, 0, uploadData.headerData.length);
				//
				if (TextUtils.isEmpty(uploadData.strFilename)) {
					if (uploadData.data != null && uploadData.data.length > 0) {
						os.write(uploadData.data, 0, uploadData.data.length);
					}
				} else {
					// 写文件数据
			...
			// p_Connect.
			// System.out.println("Connect readWebData:66666");
			rc = p_Connect.getResponseCode();
			// System.out.println("Connect readWebData:77777");
			httpResultCode = rc;
			// /////////////////////////////////////////
			// 支持cookie
			// /////////////////////////////////////////
...
//				Debug.d("totalLen:" + totalLen);
				...
				if (!bCancel) {
					...
					System.out.println("Content-Encoding:" + str);
					if (str != null && str.compareTo("gzip") == 0) {
						try {
							if (isFile) {
								result = uploadData.responseFileName.getBytes();
							} else {
								byte[] temp = ((ByteArrayOutputStream) bos)
										.toByteArray();
								System.out.println("before result:"
										+ temp.length);
								result = GZip.inflate(temp);
				...
				httpResultCode = rc;
				setToastError("服务器返回错误码：" + httpResultCode + ",请重试！");
			}
		} catch (Exception e) {
			// #if DEBUG
	...
	private boolean finished(String strUrl) {
		...
		synchronized (this) {
			if (outputTable.containsKey(strUrl)) {
				...
				// //System.out.println("Connect inputTable.size():"+inputTable.size());
				if (inputTable.containsKey(strUrl)) {
					// //System.out.println("Connect strUrl:"+strUrl);
					this.notify();
				} else {
					// System.out.println("Connect who call me?"
					// + inputTable.size() + "    " + strUrl);
					// bFinished=true;
					return true;
				}
			}
	...
	static public String urlEncode(String sUrl) {
		// System.out.println("Connect urlEncode:" + sUrl);
